...
class Problem2 {
	...
	private Solution2 bestFit(int capacity, int n) {
		if (n == 0) {
			Solution2 result = new Solution2(this.capacity, totalItems);
			if (items[0].getWeight() <= capacity)
				result.add(0, items[0]);
			return result;
		}
		...
		Solution2 included = bestFit(capacity - weightn, n - 1).add(n, items[n]);
		Solution2 excluded = bestFit(capacity, n - 1);
...
class Problem3 {
    ...
    */
     ...
     * @param totalItems Total number of Items to try to fit in knapsack 
     */
    Problem3(int capacity, int totalItems) {
        if (capacity < 0) {
            System.out.println("Error: knapsack capacity must be non-negative");
            this.capacity = 0;
        } else
     ...
     * @return Solution which maximises the value of the knapsack
     */
    private Solution3 bestFit(int capacity, int n) {
        if (n == 0) {
            Solution3 result = new Solution3(this.capacity, totalItems);
            if (items[0].getWeight() <= capacity)
                result.add(0, items[0]);
            return result;
        }
        ...
        Solution3 included = bestFit(capacity - weightn, n - 1).add(n, items[n]);
        Solution3 excluded = bestFit(capacity, n - 1);
